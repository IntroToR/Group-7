...
ratingbycoutry <- ggplot(coffee_ratings) +
  ...
  ylim(55L, 95L)
ggsave("/Users/Mengyi/Box/CPSC441 R programming/Group-7 Project/Figures/ratingbycoutry.png")

## From the ratingbycountry file, we see that the country names are not standard names. Thus, the first thing to do is to match the country names with standard names in the world map. 
country_of_origin2 <- c("Brazil", "Burundi","China","Colombia","Costa Rica","Ivory Coast","Ecuador","El Salvador","Ethiopia","Guatemala","Haiti", "Honduras"
,"India", "Indonesia","Japan","Kenya","Laos", "Malawi","Mauritius","Mexico"               ,"Myanmar","Nicaragua","Panama", "Papua New Guinea","Peru", "Philippines"               ,"Rwanda", "Taiwan","Tanzania", "Thailand","Uganda","USA","USA (Hawaii)", "Puerto Rico" ,"Vietnam", "Zambia","NA")
...
world_map2 <- world_map %>%
  select(region = country_of_origin2) %>%
  group_by(region, subregion) %>%
  summarise(long = mean(long), lat = mean(lat))

## Merge coffee rating data with the map data.
world_coffee_rating <- ratingbycountry %>%
  left_join(.,world_map2, by=c("country_of_origin" = "region"))

## Create a world map.
ggplot(world_coffee_rating, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(group = group, fill = country_of_origin))+
  geom_text(aes(label = country_of_origin), data = world_coffee_rating,  size = 3, hjust = 0.5)+
